...
)
{
  ...
  lazy val submatches: SeqView[MatchGroup, List[MatchGroup]] =
    if (matched.isEmpty) List.empty[MatchGroup].view
  ...
  def apply(m: Match): MatchGroup = apply(m, 0)._1

  def apply(m: Match, i: Int): (MatchGroup, Int) = {
    val nn = if (i == 0 || i-1 >= m.groupNames.length) None else Option(m.groupNames(i - 1))
